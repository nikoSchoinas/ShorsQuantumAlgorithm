...
#qc.u3(np.pi,0,np.pi,q)
#qc.u3(np.pi, 0, np.pi, q[0])
qc.h(q[0])
qc.measure(q[0],c[0])
qc.h(q[0])
qc.measure(q[0],c[1])
...
qc = qc.remove_final_measurements(False)
qc.draw()
...
lst = ["".join(map(str, x)) for x in lst ]
